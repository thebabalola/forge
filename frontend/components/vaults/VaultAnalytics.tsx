...
export default function VaultAnalytics({ vaultId, data = DEFAULT_DATA }: VaultAnalyticsProps) {
  const points = data;
    ...
    <div className="p-3 bg-white dark:bg-gray-900 rounded-md shadow-sm">
      <div className="flex items-center justify-between mb-2">
        ...
        <div className="text-right">
          <div className="text-lg font-semibold">${(totalAssets / 1000).toFixed(1)}k</div>
          <div className={`text-xs ${change >= 0 ? 'text-green-600' : 'text-red-600'}`}> {change >= 0 ? '+' : ''}{change.toFixed(2)}%</div>
        </div>
      </div>
      <svg width={width} height={height} viewBox={`0 0 ${width} ${height}`} className="w-full">
        <path d={path} fill="none" stroke="#4F46E5" strokeWidth={2} strokeLinejoin="round" strokeLinecap="round" />
      </svg>
      <div className="mt-2 text-xs text-gray-500">Shows TVL (total value locked) trend over time. Replace with live data via Graph / API.</div>
    </div>
  );
}
